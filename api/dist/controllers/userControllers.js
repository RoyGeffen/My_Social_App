...
export const updateUser = (req, res) => {
    ...
    jwt.verify(token, JSON.stringify(process.env.SECRET_KET), (err, userInfo) => {
        ...
        const q = "UPDATE users SET `name`=?,`username`=?,`email`=?,`city`=?,`profilePic`=?,`coverPic`=? WHERE id=? ";
        const sites_q = "UPDATE links SET `facebook`=?,`instagram`=?,`linkedIn`=?,`twitter`=?,`pinterest`=? WHERE userid=? ";
        db.query(sites_q, [
